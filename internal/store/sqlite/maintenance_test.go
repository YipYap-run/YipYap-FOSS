...
func TestMaintenanceWindowListAll(t *testing.T) {
	s := setupTestDB(t)
	ctx := context.Background()

	orgA := createTestOrg(t, s)
	orgB := createTestOrg(t, s)

	now := time.Now().UTC().Truncate(time.Second)
	a := &domain.MaintenanceWindow{
		ID: uuid.New().String(), OrgID: orgA.ID, Name: "A",
		StartAt: now, EndAt: now.Add(time.Hour), CreatedAt: now,
	}
	b := &domain.MaintenanceWindow{
		ID: uuid.New().String(), OrgID: orgB.ID, Name: "B",
		StartAt: now, EndAt: now.Add(time.Hour), CreatedAt: now,
	}
	if err := s.MaintenanceWindows().Create(ctx, a); err != nil {
		t.Fatal(err)
	}
	if err := s.MaintenanceWindows().Create(ctx, b); err != nil {
		t.Fatal(err)
	}

	all, err := s.MaintenanceWindows().ListAll(ctx)
	if err != nil {
		t.Fatal(err)
	}
	if len(all) != 2 {
		t.Fatalf("expected 2 across orgs, got %d", len(all))
	}
