...
func (s *userStore) scanUser(row *sql.Row) (*domain.User, error) {
	...
	var mfaEnforcedAt, disabledAt sql.NullString
	if err := row.Scan(&u.ID, &u.OrgID, &u.Email, &u.Name, &u.PasswordHash, &role, &u.Phone, &u.ForcePasswordChange, &u.MFAAppEnabled, &mfaEnforcedAt, &createdAt, &updatedAt, &disabledAt); err != nil {
...
func (s *userStore) scanUserFromRows(rows *sql.Rows) (*domain.User, error) {
	...
	var role, createdAt, updatedAt string
	var mfaEnforcedAt, disabledAt sql.NullString
	if err := rows.Scan(&u.ID, &u.OrgID, &u.Email, &u.Name, &u.PasswordHash, &role, &u.Phone, &u.ForcePasswordChange, &u.MFAAppEnabled, &mfaEnforcedAt, &createdAt, &updatedAt, &disabledAt); err != nil {
		return nil, err
	}
	u.Role = domain.UserRole(role)
	if mfaEnforcedAt.Valid {
