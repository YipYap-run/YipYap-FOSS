...
type Scheduler struct {
	...
	// emitter publishes monitor lifecycle CloudEvents on state transitions.
	// A nil emitter disables emission (the default); wire one in via
	// SetCloudEventEmitter before Start to avoid races.
	emitter cloudevents.Emitter
}

// SetCloudEventEmitter attaches a CloudEvent emitter to the scheduler. A nil
// emitter disables monitor.up/down/degraded/heartbeat_missed emission (the
// default). Must be called before Start to avoid races with the workers.
func (s *Scheduler) SetCloudEventEmitter(em cloudevents.Emitter) {
	s.emitter = em
}
...
func (s *Scheduler) executeCheck(ctx context.Context, m *domain.Monitor) {
	...
	// Capture the check ID before Enqueue: once enqueued, the pointer may
	// be released by the batch writer and its fields are no longer safe
	// to read from here.
	checkID := check.ID
	s.bw.Enqueue(check)
	...
	// Additive CloudEvent emission on the same transition boundaries.
	s.emitMonitorTransition(ctx, m, result, prevStatus, checkID, now)
}

// emitMonitorTransition publishes monitor.up/down/degraded.v1 CloudEvents
// on state changes. Additive to publishAlertEvent - this is the external
// eventing path, not a replacement.
func (s *Scheduler) emitMonitorTransition(ctx context.Context, m *domain.Monitor, result *Result, prevStatus domain.CheckStatus, checkID string, checkedAt time.Time) {
	if s.emitter == nil {
		return
	}
	switch result.Status {
	case domain.StatusUp:
		if prevStatus == domain.StatusDown || prevStatus == domain.StatusDegraded {
			d := types.MonitorUpV1Data{
				MonitorID: m.ID,
				CheckID:   checkID,
				LatencyMs: int64(result.LatencyMS),
				CheckedAt: checkedAt,
			}
			if err := cloudevents.EmitMonitorUp(ctx, s.emitter, m.OrgID, m.ID, d); err != nil {
				slog.Error("emit monitor.up.v1", "monitor_id", m.ID, "error", err)
			}
		}
	case domain.StatusDown:
		if prevStatus == domain.StatusUp || prevStatus == domain.StatusDegraded {
			reason := result.Error
			if reason == "" {
				reason = "unknown"
			}
			d := types.MonitorDownV1Data{
				MonitorID:     m.ID,
				CheckID:       checkID,
				FailureReason: reason,
				CheckedAt:     checkedAt,
			}
			if err := cloudevents.EmitMonitorDown(ctx, s.emitter, m.OrgID, m.ID, d); err != nil {
				slog.Error("emit monitor.down.v1", "monitor_id", m.ID, "error", err)
			}
		}
	case domain.StatusDegraded:
		if prevStatus != domain.StatusDegraded {
			reason := result.Error
			if reason == "" {
				reason = "degraded"
			}
			d := types.MonitorDegradedV1Data{
				MonitorID: m.ID,
				CheckID:   checkID,
				Reason:    reason,
				CheckedAt: checkedAt,
			}
			if err := cloudevents.EmitMonitorDegraded(ctx, s.emitter, m.OrgID, m.ID, d); err != nil {
				slog.Error("emit monitor.degraded.v1", "monitor_id", m.ID, "error", err)
			}
		}
	}
}
...
func (s *Scheduler) executeHeartbeatCheck(ctx context.Context, m *domain.Monitor) {
	...
	// Additive CloudEvent emission on heartbeat transitions.
	if s.emitter != nil {
		switch {
		case result.Status == domain.StatusDown && prevStatus == domain.StatusUp:
			var expectedBy time.Time
			var lastSeenAt time.Time
			if lastPing != nil {
				expectedBy = lastPing.CheckedAt.Add(gracePeriod)
				lastSeenAt = lastPing.CheckedAt
			} else {
				// First-check-no-ping case: no prior ping to anchor the
				// window, so the missed deadline is "now" and last_seen
				// stays zero (omitempty in the catalog).
				expectedBy = now
			}
			d := types.MonitorHeartbeatMissedV1Data{
				MonitorID:  m.ID,
				ExpectedBy: expectedBy,
				LastSeenAt: lastSeenAt,
			}
			if err := cloudevents.EmitMonitorHeartbeatMissed(ctx, s.emitter, m.OrgID, m.ID, d); err != nil {
				slog.Error("emit monitor.heartbeat_missed.v1", "monitor_id", m.ID, "error", err)
			}
		case result.Status == domain.StatusUp && prevStatus == domain.StatusDown:
			d := types.MonitorUpV1Data{
				MonitorID: m.ID,
				CheckID:   checkID,
				// LatencyMs is not meaningful for heartbeat monitors.
				CheckedAt: now,
			}
			if err := cloudevents.EmitMonitorUp(ctx, s.emitter, m.OrgID, m.ID, d); err != nil {
				slog.Error("emit monitor.up.v1 (heartbeat)", "monitor_id", m.ID, "error", err)
			}
		}
	}
}
