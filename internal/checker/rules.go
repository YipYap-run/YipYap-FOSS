...
func matchesRule(rule domain.MonitorMatchRule, statusCode int, body string, headers http.Header) bool {
	...
	if rule.HeaderMatch != "" {
		...
		if rule.HeaderValue == "" {
			// No expected value: the header must simply be present.
			if headerVal == "" {
				return false
			}
		} else {
			switch rule.HeaderMatchMode {
			case "contains":
				if !strings.Contains(headerVal, rule.HeaderValue) {
					return false
				}
			case "regex":
				matched, err := regexp.MatchString(rule.HeaderValue, headerVal)
				if err != nil || !matched {
					return false
				}
			default: // "equals" (also the legacy empty mode)
				if headerVal != rule.HeaderValue {
					return false
				}
			}
		}
