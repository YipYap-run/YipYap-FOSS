...
func setupDispatcher(msgBus bus.Bus, _ store.Store, decrypt notify.DecryptFunc, _ *config.Config) (*notify.Dispatcher, *providers.CloudEventHTTP) {
	...
	cehttp := providers.NewCloudEventHTTP(decrypt)
	d.Register(cehttp, 100)
